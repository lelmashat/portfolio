  ...
  if (root_ == nullptr) {
      ...
      while (current != nullptr) {
          ...
          if (keyValuePair.first < current->getKey()) {
              ...
              current->setValue(keyValuePair.second); // Key already exists, update value
...
void BinarySearchTree<Key, Value>::remove(const Key& key) {
    ...
    // Case: The node to remove has two children.
    if (nodeToRemove->getLeft() != nullptr && nodeToRemove->getRight() != nullptr) {
        Node<Key, Value>* predNode = predecessor(nodeToRemove);

        // If predecessor node is not the direct left child, handle its subtree.
        if (predNode->getParent() != nodeToRemove) {
            if (predNode->getLeft() != nullptr) {
                predNode->getLeft()->setParent(predNode->getParent());
            }
            predNode->getParent()->setRight(predNode->getLeft());
            predNode->setLeft(nodeToRemove->getLeft());
            nodeToRemove->getLeft()->setParent(predNode);
        }

        // Connect the predecessor with the nodeToRemove's right child.
        if (nodeToRemove->getParent() == nullptr) {
            root_ = predNode;
        } 
        else if (nodeToRemove == nodeToRemove->getParent()->getLeft()) {
            nodeToRemove->getParent()->setLeft(predNode);
        } 
        
        else { nodeToRemove->getParent()->setRight(predNode); }
        predNode->setParent(nodeToRemove->getParent());
        predNode->setRight(nodeToRemove->getRight());
        nodeToRemove->getRight()->setParent(predNode);
    }
   
    // Case: The node to remove has only one child or no child.
    else {
        
        Node<Key, Value>* child = (nodeToRemove->getLeft() != nullptr) ? nodeToRemove->getLeft() : nodeToRemove->getRight();
        
        if (nodeToRemove->getParent() == nullptr) { root_ = child; }
            
        else if (nodeToRemove == nodeToRemove->getParent()->getLeft()) {
            nodeToRemove->getParent()->setLeft(child);
        } 
        
        else { nodeToRemove->getParent()->setRight(child); }
        
        if (child != nullptr) { child->setParent(nodeToRemove->getParent()); }
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    ...
    // If the current node has a left subtree, then the predecessor is the rightmost node in that subtree
    ...
    // If the current node doesn't have a left subtree, then the predecessor is an ancestor of the current node
