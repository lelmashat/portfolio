...
public:
    ...
    // To be completed (see for change)
    HASH_INDEX_T next() 
    {
      if (this->numProbes_ >= this->m_) {
          return this->npos;
...
{
    mIndex_ = 0; // Initialize mIndex to the first capacity in CAPACITIES array
    HASH_INDEX_T initialCapacity = CAPACITIES[mIndex_];
    table_.resize(initialCapacity, nullptr); // Initialize table_ with initialCapacity elements
}
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
    // Clean up dynamically allocated HashItems
...
{
    size_t count = 0;
    for(typename std::vector<HashItem*>::const_iterator it = table_.cbegin(); it != table_.cend(); ++it) {
        if(*it && !(*it)->deleted) {
            count++;
        }
    }
...
{
    size_t htSize = CAPACITIES[mIndex_];
    mIndex_++;
    size_t newSize = CAPACITIES[mIndex_];
    std::vector<HashItem*> newHT(newSize, nullptr);
    std::vector<HashItem*> oldHT = table_;
    table_ = newHT;

    for(size_t i = 0; i < htSize; ++i){
      HashItem* currItem = oldHT[i];
      if(currItem != nullptr){
        if(currItem->deleted){
          delete currItem;
        }
        else{
          HASH_INDEX_T newIndex = probe(currItem->item.first);
          table_[newIndex] = currItem;
        }
      }
    }
    table_->deleted = 0;
